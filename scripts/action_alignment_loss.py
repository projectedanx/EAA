"""
Action-Alignment Loss Implementation.

This module provides the differentiable Action-Alignment Loss (Regret Minimization)
in PyTorch, bridging the 'thought-action' gap by mathematically penalizing the agent's
policy if it deviates from the optimal Best Response calculated based on its descriptive
prediction of the opponent's strategy.
"""
import torch
...
class ActionAlignmentLoss(nn.Module):
    ...
    Args:
        payoff_matrix (torch.Tensor): Float tensor of shape (num_agent_actions, num_opponent_actions)
                                      representing the utility values for the agent.
        use_smooth (bool, optional): If True, applies log-sum-exp (Boltzmann) to compute the
                                     oracle expected utility, ensuring dense gradient flow.
                                     Defaults to True.
        temperature (float, optional): Scaling factor (tau) for the smooth best-response calculation.
                                       Defaults to 0.1.
    """
    def __init__(self, payoff_matrix: torch.Tensor, use_smooth: bool = True, temperature: float = 0.1):
        """
        Initializes the ActionAlignmentLoss module.
    ...
    def forward(self, agent_logits: torch.Tensor, predicted_opponent_logits: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            agent_logits (torch.Tensor): Raw logits from the execution head (Head B)
                                   of shape (batch_size, num_agent_actions).
            predicted_opponent_logits (torch.Tensor): Raw logits from the ToM prediction head
                                                (Head A) of shape (batch_size, num_opponent_actions).

        Returns:
            torch.Tensor: Scalar tensor representing the batch mean alignment loss.
        """
